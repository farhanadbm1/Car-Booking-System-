    ...
    public class BookingsController : ControllerBase
    {
        ...
        public async Task<IEnumerable<BookingCalendarDto>> GetCalendarBookings([FromQuery] BookingFilterDto input)
        {
            var bookings = await _context.Bookings
                .Where(b => b.RepeatOption == input.RepeatOption)
                .Include(b => b.Car) // Ensure car details are included
                .ToListAsync();
            ...
            foreach (var booking in bookings)
            {
                ...
                // Loop to handle repeat options
                while (currentDate <= (booking.EndRepeatDate ?? booking.BookingDate))
                {
                    // Check if the current date matches the specified days to repeat on
                    if (booking.RepeatOption == RepeatOption.Weekly && booking.DaysToRepeatOn.HasValue)
                    {
                        var dayOfWeek = (DaysOfWeek)(1 << (int)currentDate.DayOfWeek);
                        if ((booking.DaysToRepeatOn.Value & dayOfWeek) == 0)
                        {
                    ...
                    calendar.Add(new BookingCalendarDto
                    {
                        ...
                        RepeatOption = booking.RepeatOption // Set this property
                    });

                    // Update currentDate based on the repeat option
...
public async Task<IActionResult> PostBooking([FromBody] CreateUpdateBookingDto bookingDto)
{
    ...
    // Convert DateTime to DateOnly (if needed)
    var bookingDate = DateOnly.FromDateTime(DateTime.Parse(bookingDto.BookingDate.ToString()));

    var booking = new Booking
    {
        Id = Guid.NewGuid(),
        BookingDate = bookingDate, // Ensuring DateOnly is properly assigned
        StartTime = bookingDto.StartTime,
        EndTime = bookingDto.EndTime,
        RepeatOption = bookingDto.RepeatOption,
        EndRepeatDate = bookingDto.EndRepeatDate.HasValue 
            ? DateOnly.FromDateTime(DateTime.Parse(bookingDto.EndRepeatDate.Value.ToString())) 
            : (DateOnly?)null,
    ...
    var conflictingBookings = await _context.Bookings
        .Where(b => b.CarId == booking.CarId && b.BookingDate == booking.BookingDate &&
    ...
    {
        return Conflict("Booking time conflicts with existing bookings.");
    }

    await _context.Bookings.AddAsync(booking);
    await _context.SaveChangesAsync();

    return CreatedAtAction(nameof(GetCalendarBookings), new { id = booking.Id }, bookingDto);
}
